...
public class EvenementService implements IService<Evenement> {
    ...
    public void addEntity(Evenement evenement) throws SQLException {
        String requete = "INSERT INTO evenement (id_mentor, titre, contenu, lieu, lieu_latitude, lieu_longitude, image_url, date_debut, date_fin) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if (evenement.getLieuLatitude() != null) {
            st.setDouble(5, evenement.getLieuLatitude());
        } else {
            st.setNull(5, Types.DOUBLE);
        }
        
        if (evenement.getLieuLongitude() != null) {
            st.setDouble(6, evenement.getLieuLongitude());
        } else {
            st.setNull(6, Types.DOUBLE);
        }
        
        st.setString(7, evenement.getImageUrl());
        st.setTimestamp(8, Timestamp.valueOf(evenement.getDateDebut()));
        st.setTimestamp(9, Timestamp.valueOf(evenement.getDateFin()));
    ...
    public void updateEntity(Evenement evenement) throws SQLException {
        ...
        st.setTimestamp(8, Timestamp.valueOf(evenement.getDateDebut()));
        st.setTimestamp(9, Timestamp.valueOf(evenement.getDateFin()));
        st.setInt(10, evenement.getIdEvenement());
