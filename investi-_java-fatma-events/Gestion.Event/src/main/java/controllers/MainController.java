...
public class MainController {
    ...
    void initialize() {
        ...
        // Initialize theme button and apply current theme
        if (btnToggleTheme != null) {
            btnToggleTheme.setText(themeManager.getThemeButtonText());
            // Apply theme after scene is ready
            btnToggleTheme.sceneProperty().addListener((obs, oldScene, newScene) -> {
                if (newScene != null) {
    ...
    void exporterEvenementsPDF() {
        ...
        if (file != null) {
            try {
                List<Evenement> events = tabEvent.getItems();
                boolean success = edu.Investi.tools.PDFExporter.exportEventsToPDF(events, file.getAbsolutePath());
                if (success) {
                    afficherSucces("Export réussi", "Fichier créé : " + file.getName());
                    FileOpener.openFile(file);
                } else {
                    afficherErreur("Erreur", "Export échoué", "Impossible de créer le fichier");
                }
            } catch (Exception ex) {
                afficherErreur("Erreur", "Export échoué", ex.getMessage());
            }
    ...
    void exporterInscriptionsPDF() {
        ...
        if (file != null) {
            try {
                List<Inscription> inscrs = tabInscription.getItems();
                boolean success = edu.Investi.tools.PDFExporter.exportInscriptionsToPDF(inscrs, file.getAbsolutePath());
                if (success) {
                    afficherSucces("Export réussi", "Fichier créé : " + file.getName());
                    FileOpener.openFile(file);
                } else {
    ...
    void toggleTheme(ActionEvent e) {
        try {
            ...
            System.err.println("Error toggling theme: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
}
