...
public class MainController {
    ...
    // Navigation
    @FXML private Button btnNavEvenement, btnNavInscription, btnNavStats, btnNavCalendrier;
    @FXML private VBox vueEvenements, vueInscriptions, vueStats, vueCalendrier;
    @FXML private Label lblDateTime;

    // Événements
    @FXML private TableView<Evenement> tabEvent;
    @FXML private TableColumn<Evenement, Integer> colEventId, colEventMentor;
    @FXML private TableColumn<Evenement, String> colEventTitre, colEventLieu;
    @FXML private TableColumn<Evenement, LocalDateTime> colEventDebut, colEventFin;
    @FXML private TableColumn<Evenement, Void> colEventActions;
    @FXML private TextField searchEvenement;
    @FXML private ComboBox<String> filtreLieuEvent, filtreMentorEvent;
    @FXML private Label lblCountEvent;

    private FilteredList<Evenement> filteredEvents;

    // Inscriptions
    @FXML private TableView<Inscription> tabInscription;
    @FXML private TableColumn<Inscription, Integer> colInscrId, colInscrUser, colInscrEvent;
    @FXML private TableColumn<Inscription, String> colInscrStatut;
    @FXML private TableColumn<Inscription, LocalDateTime> colInscrDate;
    @FXML private TableColumn<Inscription, Void> colInscrActions;
    @FXML private TextField searchInscription;
    @FXML private ComboBox<String> filtreStatutInscr, filtreEventInscr;
    @FXML private Label lblCountInscr;

    private FilteredList<Inscription> filteredInscriptions;

    // Stats
    @FXML private Label lblStatEvents, lblStatInscr, lblStatConfirm, lblStatAttente;
    @FXML private PieChart chartStatut;
    @FXML private BarChart<String, Number> chartEvents;
    ...
    private void activerOnglet(Button actif, VBox vue) {
        ...
        vueEvenements.setVisible(false); vueEvenements.setManaged(false);
        vueInscriptions.setVisible(false); vueInscriptions.setManaged(false);
        vueStats.setVisible(false); vueStats.setManaged(false);
        vueCalendrier.setVisible(false); vueCalendrier.setManaged(false);
    ...
    void rechercherInscriptions() {
        ...
        filteredInscriptions.setPredicate(ins -> {
            if (query.isEmpty()) return true;
            return String.valueOf(ins.getIdUtilisateur()).contains(query) ||
        ...
        if (file != null) {
            try (FileWriter w = new FileWriter(file)) {
                ...
                for (Inscription i : tabInscription.getItems()) {
                    w.write(String.format("%d,%d,%d,%s,%s\n",
                            i.getIdInscription(), i.getIdUtilisateur(), i.getIdEvenement(),
    ...
    private void supprimerInscription(Inscription ins) {
        Alert a = new Alert(Alert.AlertType.CONFIRMATION);
        a.setTitle("Confirmation");
        a.setHeaderText("Supprimer inscription User " + ins.getIdUtilisateur() + " ?");
        a.showAndWait().ifPresent(btn -> {
            if (btn == ButtonType.OK) {
                try {
    ...
    private void afficherErreur(String titre, String header, String msg) {
        ...
        a.setTitle(titre); a.setHeaderText(header); a.setContentText(msg); a.showAndWait();
    }
