...
public class CollaborationService {
    ...
    public Collaboration createCollaboration(Collaboration c) throws SQLException {
        String query = "INSERT INTO collaboration (project_id, investor_id, start_date, status, health_score, default_probability, fairness_score, fairness_status, ideal_equity, equity_deviation) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement pst = getCnx().prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            pst.setInt(1, c.getProjectId());
            pst.setInt(2, c.getInvestorId());
            pst.setTimestamp(3, c.getStartDate() != null ? new Timestamp(c.getStartDate().getTime())
                    : new Timestamp(System.currentTimeMillis()));
            pst.setString(4, c.getStatus() != null ? c.getStatus() : "ACTIVE");
            pst.setDouble(5, c.getHealthScore() > 0 ? c.getHealthScore() : 100.0);
            pst.setDouble(6, c.getDefaultProbability());
            pst.setDouble(7, c.getFairnessScore() > 0 ? c.getFairnessScore() : 100.0);
            pst.setString(8, c.getFairnessStatus() != null ? c.getFairnessStatus() : "BALANCED");
    ...
    public Collaboration getCollaborationByInvestment(int projectId, int investorId) throws SQLException {
        String query = "SELECT * FROM collaboration WHERE project_id = ? AND investor_id = ?";
        try (PreparedStatement pst = getCnx().prepareStatement(query)) {
            pst.setInt(1, projectId);
            pst.setInt(2, investorId);
